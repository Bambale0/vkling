  ...
  /api/v1/jobs/createTask:
    post:
      ...
      description: >-
        ...
        - **双生成模式**：在 `std`（标准分辨率）和 `pro`（更高分辨率）模式之间选择
        ...
        专业模式提供更高分辨率的输出，但可能需要更长的生成时间并消耗更多积分。
        ...
        - **模式选择**：当质量重要时使用 `pro` 模式进行最终输出，在需要快速迭代时使用 `std` 模式
                ...
                input:
                  ...
                  properties:
                    ...
                    mode:
                      type: string
                      description: >-
                        生成模式。std 拥有标准的分辨率，pro 拥有更高的分辨率。


                        分辨率对应关系：

                        - **std 模式**：16:9（1280×720）、9:16（720×1280）、1:1（720×720）

                        - **pro
                        模式**：16:9（1920×1080）、9:16（1080×1920）、1:1（1080×1080）
                      enum:
                        - std
                        - pro
                      default: pro
